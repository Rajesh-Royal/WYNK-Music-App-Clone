...
export default function ToggleDarkMode({toggleTheme} : any): ReactElement {
  ...
  if (localStorage.WYNK_Theme === "dark" || (!("WYNK_Theme" in localStorage) && window.matchMedia("(prefers-color-scheme: dark)").matches)) {
  ...
  return (
    <>
        {
            dark ?
            <img src={DaySun} className="w-5 cursor-pointer transition-all duration-500" alt="sun icon"
            onClick={() => {
                setDark(false);
                localStorage.WYNK_Theme = "light";
                toggleTheme();
            }}/> :
            <img src={NightMoon} className="w-5 cursor-pointer" alt="moon icon"
            onClick={() => {
                setDark(true);
