        ...
        {({ open }) => (
          <>
            <Popover.Button
              ...
              <span>MY MUSIC</span>
              <ChevronDownIcon className={classNames(open ? "text-gray-600" : "text-gray-400", "ml-2 h-5 w-5 group-hover:text-gray-500 dark:text-gray-300 text-sm")} aria-hidden="true" />
            </Popover.Button>
            ...
            <Transition
              ...
              <Popover.Panel static className="absolute z-10 -ml-4 mt-3 transform px-2 w-screen max-w-md sm:px-0 lg:ml-0 lg:left-1/2 lg:-translate-x-1/2" data-testid="my-music-dropdown">
                <div className="rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 overflow-hidden">
                  <div className="relative grid gap-6 bg-white px-5 py-6 sm:gap-8 sm:p-8 dark:bg-gray-850">
                    {MyMusic.map((item) => (
                      <a key={item.name} href={item.href} className="-m-3 p-3 flex items-start rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800">
                        <item.icon className="flex-shrink-0 h-6 w-6 text-red-600 dark:text-red-700" aria-hidden="true" />
                        <div className="ml-4">
